...
class AiTeamReportNarrativeService
{
    ...
    public function generateNarrative(array $reportData, array $options = []): array
    {
        ...
        try {
            ...
            if ($this->isValidNarrativePayload($parsedJson)) {
                // Ensure agent review items have required verbatim structure
                foreach ($parsedJson['agent_reviews'] as &$arItem) {
                    $arItem['verbatim_analysis'] = (string) ($arItem['verbatim_analysis'] ?? $arItem['assessment'] ?? 'Análisis no disponible.');
                    $arItem['strengths'] = is_array($arItem['strengths'] ?? null) ? $arItem['strengths'] : [];
                    $arItem['friction_points'] = is_array($arItem['friction_points'] ?? null) ? $arItem['friction_points'] : [];
                }
                unset($arItem);
        ...
        return [
            'narrative' => $this->buildDeterministicNarrative($reportData),
            'tokens_used' => 0,
    ...
    public function buildDeterministicNarrative(array $reportData): array
    {
        ...
        foreach ($agentReviews as $ar) {
            ...
            $action = 'Mantener acompañamiento y seguimiento rutinario.';

            if ($aStatus === 'critical') {
                $assessment .= 'Desempeño en rango crítico con oportunidades severas de satisfacción.';
                $action = 'Programar sesión urgente 1 a 1 de calibración de llamadas y plan de acompañamiento.';
                ...
                $assessment .= 'Muestra insuficiente para concluir tendencia individual.';
                $action = 'Priorizar monitoreo adicional de llamadas para evaluar calidad de manera representativa.';
            } else {
                $assessment .= 'Rendimiento alineado con las metas operacionales.';
    ...
    protected function buildSystemInstruction(?array $goals = null): string
    {
        return 'Eres un Director Ejecutivo de Operaciones y Calidad de Clientes (Customer Experience y Voice of Customer). '
            ...
            ."3. Devuelve OBLIGATORIAMENTE un JSON válido sin texto previo ni posterior, con la siguiente estructura:\n"
            ."{\n"
